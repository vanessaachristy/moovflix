...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($password, $hashedPassword)) {
            header("Location: booking-details/seating-page/index.php");
        } else {
            $loginFailed = true;
        }
    } else {
                    ...
                    <div class="input-field">
                        <form action="" method="post" id="userForm" onsubmit="return validateForm()">
                            <div class="form-group">
                                ...
                                <input class="input-fields <?php if ($loginFailed) echo 'error-border'; ?>" type="email" id="userEmail" name="email" placeholder="Enter your email here" required size="30" oninput="validateEmail()">
                                <div class="error" id="invalidEmail">Please enter a valid email address.</div>
                            </div>
                            <div class="form-group">
                                <label class="fields-title">Password</label>
                                <input class="input-fields <?php if ($loginFailed) echo 'error-border'; ?>" id="userPassword" type="password" name="password" placeholder="Enter your password here" required size="30">
                        ...
                        <h3>Don't have an account? <span class="sign-up-link"><a href="signup.php">Sign Up</a></span></h3>
                    </div>
                </div>
        </section>
